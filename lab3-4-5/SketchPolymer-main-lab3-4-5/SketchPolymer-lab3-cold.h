...
public:
    ...
    Stage1(uint32_t memory, uint32_t dim): d(dim) {
        ...
        m1 = m*0.65*2; //4bit
        m2 = m*0.35*4/3; //6bit
        counter1 = new uint8_t* [d];
        counter2 = new uint8_t* [d];
        for (int i = 0; i < d; ++i) {
            counter1[i] = new uint8_t [m1];
            counter2[i] = new uint8_t [m2];
            memset(counter1[i], 0, m1 * sizeof(uint8_t));
            memset(counter2[i], 0, m2 * sizeof(uint8_t));
        }
    }
    ~Stage1() {
        for (int i = 0; i < d; ++i) {
    ...
    void insert(ID_TYPE id) {
        ...
        for (int i = 0; i < d; ++i) {
            uint32_t index = hash(id, i) % m1;
            min_val = MIN((counter1[i][index]&0xf),min_val);
        }
        if(min_val == 0xf)
        {
            for(int i=0;i<d;i++)
            {
                uint32_t index = hash(id,7*i) % m2;
                if(counter2[i][index] < 0x3f) counter2[i][index]++;
            }
        }else{
            min_val++;
            for(int i=0;i<d;i++)
            {
                uint32_t index = hash(id,i) % m1;
                counter1[i][index] = MAX(min_val,counter1[i][index]);
            }
        }
    }
    uint32_t query(ID_TYPE id) {
        ...
        for (int i = 0; i < d; ++i) {
            uint32_t index = hash(id, i) % m1;
            min_val = MIN(min_val, counter1[i][index]);
        }
        if(min_val==0xf)
        {
            uint32_t min_2 = 1<<30;
            for (int i = 0; i < d; ++i) {
                uint32_t index = hash(id, 7*i) % m2;
                min_2 = MIN(min_2, counter2[i][index]&0x3f);
            }
            min_val+=min_2;
        }
        return min_val;
