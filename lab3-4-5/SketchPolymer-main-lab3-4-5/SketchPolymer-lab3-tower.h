...
public:
    ...
    void insert(ID_TYPE id) {
        uint32_t index1 = hash(id, 10) % (4*m);
        uint32_t index2 = hash(id, 11) % (2*m);
        uint32_t index3 = hash(id, 12) % m;
        uint8_t c1 = (counter[0][index1/4] & (0x3<<( (index1%4)<<1 ))) >> ( (index1%4)<<1 );
        uint8_t c2 = (counter[1][index2/2] & (0xf<<( (index2%2)<<2 ))) >> ( (index2%2)<<2 );
        uint8_t c3 = counter[2][index3];
        if(c1 < 0x3) c1++;
        if(c2 < 0xf) c2++;
        if(c3 < 0xff) c3++;
        counter[0][index1/4] &= ~(0x3<<( (index1%4)<<1 ));
        counter[0][index1/4] |= (c1<<( (index1%4)<<1 ));
        counter[1][index2/2] &= ~(0xf<<( (index2%2)<<2 ));
        counter[1][index2/2] |= (c2<<( (index2%2)<<2 ));
        counter[2][index3] = c3;
    }
    uint32_t query(ID_TYPE id) {
        ...
        uint8_t c2 = (counter[1][index2/2] & (0xf<<( (index2%2)<<2 ))) >> ( (index2%2)<<2 );
        uint8_t c3 = counter[2][index3];
        if(c1 < 0x3) frequency = MIN(frequency, c1);
        if(c2 < 0xf) frequency = MIN(frequency, c2);
        frequency = MIN(frequency, c3);
        return frequency;
    }
private:
    uint32_t d;
