...
public:
    ...
    Stage2(uint32_t memory, uint32_t dim): d(dim) {
        m = memory * 1024 / (sizeof(stage2_block)) / d;
        frq_val = new stage2_block* [d];
        for (int i = 0; i < d; ++i) {
            frq_val[i] = new stage2_block [m];
            memset(frq_val[i], 0, m * sizeof(stage2_block));
        }
    }
    ~Stage2() {
        for (int i = 0; i < d; ++i) {
    ...
    static inline bool stage2_block_cmp(struct stage2_block a,struct stage2_block b)
    {
    ...
    void insert(ID_TYPE id, uint32_t t) {
        for (int i = 0; i < d; ++i) {
            ...
            int32_t sign = hash(id, 37 + i) % 2;
            frq_val[i][index].frequency += sign ? 1 : -1;
            frq_val[i][index].value = MAX(frq_val[i][index].value, t);
        }
    }
    void query(ID_TYPE id, uint32_t* f, uint32_t* t) {
        struct stage2_block *result = new stage2_block[d];
        memset(result,0,d*sizeof(stage2_block));
        for (int i = 0; i < d; ++i) {
            uint32_t index = hash(id, 33 + i) % m;
            int32_t sign = hash(id, 37 + i) % 2;
            result[i].frequency = sign ? frq_val[i][index].frequency : -frq_val[i][index].frequency;
            result[i].value = frq_val[i][index].value;
        }
        std::sort(result,result+d,stage2_block_cmp);
        int mid = d/2;
        if(d%2==0) {
            *f = (result[mid].frequency+result[mid-1].frequency)/2;
            *t = (result[mid].value+result[mid-1].value)/2;
        }
        else {
            *f = result[mid].frequency;
            *t = result[mid].value;
        }
        delete[] result;
    }
...
public:
    ...
    Stage3(uint32_t memory, uint32_t dim): d(dim) {
        m = memory * 1024 / sizeof(int16_t) / d;
        counter = new int16_t* [d];
        for (int i = 0; i < d; ++i) {
            counter[i] = new int16_t [m];
    ...
    void insert(ID_TYPE id, uint32_t t) {
        for (int i = 0; i < d; ++i) {
            ...
            int32_t sign = hash(id*t,11*i) % 2;
            if (sign == 1 && counter[i][index] != std::numeric_limits<int16_t>::max())
                counter[i][index]++;
            else if(sign == 0 && counter[i][index] != std::numeric_limits<int16_t>::min())
                counter[i][index]--;
        }
    }
    uint32_t query(ID_TYPE id, uint32_t t) {
        int32_t *result = new int32_t[d];
        for (int i = 0; i < d; ++i) {
            uint32_t index = hash(id * t, i) % m;
            int32_t sign = hash(id * t,11 * i) % 2;
